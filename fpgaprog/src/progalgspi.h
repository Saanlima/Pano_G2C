...
class ProgAlgSpi
{
  private:
    ...
    unsigned int BulkErase; // Max time in ms to do a chip erase
    unsigned int SectorErase; // Max time in ms to do a sector erase
    unsigned int PageProgram; // Max time in ms to do a page programming
    ...
    bool Spi_PartialErase(int length, int page_offset, bool verbose=false);
    bool Spi_Write(const byte *write_data, int length, int page_offset, bool verbose=false);
    bool Spi_Verify(const byte *verify_data, int length, int page_offset, bool verbose);
  ...
  public:
    ...
    bool ProgramSpi(BitFile &file, Spi_Options_t options, int offset);
    bool EraseSpi();
};
